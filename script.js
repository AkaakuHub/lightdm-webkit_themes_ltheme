...
function show_prompt(text) {
  ...
  if (!isVisible(password_container)) {
    ...
    const bg_mask = document.querySelector(".bg_mask");
    bg_mask.classList.add("bg_mask_darker");
...
function show_users() {
  ...
  const bg_mask = document.querySelector(".bg_mask");
  bg_mask.classList.remove("bg_mask_darker");
}
...
document.addEventListener("keydown", function(event) {
  ...
  if (event.code === "Space" || event.code === "Enter") {
    if (!password_container.classList.contains("hidden")) { return; }
    let firstUser = document.querySelector(".user");
    if (firstUser) {
        bg_mask.classList.add("bg_mask_darker");
        event.preventDefault();
        user_clicked({ currentTarget: firstUser, stopPropagation: () => {} });
